...
import org.openo.crossdomain.commsvc.common.log.impl.LogResult;
import org.openo.crossdomain.commsvc.jobscheduler.dao.inf.IJobDao;

/**
 * Query progress job for updating.<br/>
 * 
 * @author
 * @version crossdomain 0.5 2016-3-19
 */
public class UpdateQueryProgressJob extends QueryProgressJob {
    ...
    public void handleComplete(ServiceModel serviceModel) {
        try {
            ...
            logger.info("UpdateQueryProgressJob : completed");

            OperationLog.writeLogString(OperationLog.OPER_TYPE.UPDATE_TASK_OPERATION, serviceModel.getService_id(),
                    AuditResult.SUCCESS, serviceModel, LogSeverity.WARNING);
        } catch(ServiceException e) {
            logger.error("UpdateQueryProgressJob : update serviceModelDao failed, error {}", e);
        }
    }
    ...
    public void handleFailed(ServiceModel serviceModel, String reason) {
        ...
        try {
            ...
            if(!StringUtils.hasLength(reason)) {
                reason = "serviceDecomposer update failed";
            }
            operationManager.updateServiceOperation(serviceOperation, serviceModel.getParameters(),
                    OperationConstant.OPERATION_RESULT_NUM.FAILED, reason);
            logger.info("UpdateQueryProgressJob : failed");

            OperationLog.writeLogString(OperationLog.OPER_TYPE.UPDATE_TASK_OPERATION, serviceModel.getService_id(),
                    AuditResult.FAILURE, serviceModel, LogSeverity.WARNING);
        } catch(ServiceException e) {
            logger.error("UpdateQueryProgressJob : update serviceModelDao failed, error {}");
        }
    }
